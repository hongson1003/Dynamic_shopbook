...
const Payment = ({ items }: { items: ItemCartModel[] }) => {
  ...
  return (
    <></> || <div className="flex w-full justify-end bg-white p-2 shadow-md dark:bg-gray-800 fixed inset-x-0 bottom-0">
      <div className='flex flex-1 flex-col mr-4'>
        <p className='text-sm font-semibold text-right'>Tổng thanh toán</p>
        <p className='text-md font-semibold text-red-600 text-right'>{formatCurrencyVND(total)}</p>
      </div>
      <button className="mt-auto bg-[--bg-header] p-1 p-4 py-3 text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
